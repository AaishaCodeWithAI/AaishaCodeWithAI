# Hi, I'm Aaisha 👋  
### Aspiring Developer | Cyber Security Student | Web, AI & Cloud Enthusiast

💻 I’m currently pursuing my Bachelor's of Computer Science specializing in **Cyber Security and Networking**
💻 I'm also learning about **Web Development and AI**
🚀 I’m working on improving my skills through real projects  
📚 Second Semester CS Student  
🎯 My goal: Build strong projects + master full-stack development + become tech professional.

---

### 🛠️ Tech Stack Skills & Interests
- **Currently Learning / Exploring Languages:** Python, Java, HTML, CSS, JavaScript  
- **Tools I Use:** VS Code, Git, VirtualBox  
- **Areas of Interest:** Web Development, Cloud Computing, AI, Cybersecurity

---

### 📌 Current Goals
- Build 10+ beginner-to-intermediate projects  
- Learn Git + GitHub deeply  
- Create personal portfolio website  
- Contribute to open-source projects  

---

### 📂 Featured Projects
| Project | Description | Tech |
|--------|-------------|------|
| Personal Portfolio (In Progress) | Building my first personal website to showcase projects | HTML, CSS, JS |
| Simple Login System (Learning) | Exploring backend development concepts | Python |
| Notes App (Learning) | Practicing frontend coding and interactivity | JavaScript |

---

### 📫 Connect with Me
- LinkedIn (coming soon)  
- Email: jhaaaisha78@gmail.com
